...
#####################################################################################################
    ...
    raw_radforc = CSV.File(normpath(@__DIR__,"..","data", (rcp_scenario * "_radiativeforcing.csv")), skipto=60, header = 59) |> DataFrame
    raw_emiss   = CSV.File(normpath(@__DIR__,"..","data", (rcp_scenario * "_emission.csv")), header = 37, skipto=38) |> DataFrame
    raw_conc    = CSV.File(normpath(@__DIR__,"..","data", (rcp_scenario * "_concentration.csv")), header = 38, skipto=39) |> DataFrame
    # Note that it is the Kaya data which we are now calling 'ssp'.
    raw_ssp      = DataFrame(CSV.File(normpath(@__DIR__,"..", "data", "Kaya_Eli.csv"), skipto=5, header = 4))
...
function run_model(co2_policy)
    ...
    output = DataFrame(years = [start_year:1:end_year;], 
